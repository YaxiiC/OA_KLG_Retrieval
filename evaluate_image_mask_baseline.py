r"""
Evaluate an image+mask baseline model (train_image_mask_baseline.py).

Saves:
- predictions_eval.csv (case_id, pred_class, prob_0..prob_4)
- metrics_eval.json (if labels provided)
python evaluate_image_mask_baseline.py \
  --model-dir "training_logs_image_mask_baseline" \
  --images-dir "/home/yaxi/nnUNet/nnUNet_raw/Dataset360_oaizib/imagesTs" \
  --masks-dir "/home/yaxi/nnUNet/nnUNet_results/predicted_masks/test" \
...
python evaluate_image_mask_baseline.py \
  --model-dir "training_logs_image_mask_baseline_ordinal_eff_b_concat" \
  --images-dir "/home/yaxi/nnUNet/nnUNet_raw/Dataset360_oaizib/imagesTs" \
  --masks-dir "/home/yaxi/nnUNet/nnUNet_results/predicted_masks/test" \
  --radiomics-csv "/home/yaxi/YaxiiC-OA_KLG_Retrieval/output_test/radiomics_results_wide.csv" \
  --klgrade-csv "/home/yaxi/YaxiiC-OA_KLG_Retrieval/subInfo_test.xlsx" \
...
from data_loader import load_klgrade_labels, load_radiomics_wide_format
from training_utils import compute_metrics, ordinal_logits_to_proba, ordinal_logits_to_pred
from train_image_mask_baseline import ImageMaskDataset, ImageMaskModel
...
def _has_pair(
    ...
    for ext in (".nii.gz", ".nii"):
        if (images_dir / f"{case_id}{image_suffix}{ext}").exists() and (
            masks_dir / f"{case_id}{mask_suffix}{ext}"
        ).exists():
            return True
    return False
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate image+mask baseline model",
    ...
    parser.add_argument("--masks-dir", type=str, required=True, help="Masks directory")
    ...
    parser.add_argument("--mask-mode", type=str, default=None,
                        choices=["concat", "mul"],
                        help="Override mask mode (default: from checkpoint)")
    ...
    mask_mode = args.mask_mode or checkpoint.get("mask_mode") or ckpt_args.get("mask_mode") or "concat"

    logger.info(
        f"Arch: {arch} | label_mode: {label_mode} | label_num_classes={label_num_classes} | "
        f"ordinal={bool(ordinal)} | out_dim={out_dim} | mask_mode={mask_mode}"
    )
    ...
    # Drop cases without matching image+mask files (like training does).
    images_dir = Path(args.images_dir)
    masks_dir = Path(args.masks_dir)
    before = len(case_ids)
    case_ids = [
        cid for cid in case_ids
        if _has_pair(images_dir, masks_dir, cid, args.image_suffix, args.mask_suffix)
    ]
    dropped = before - len(case_ids)
    if dropped > 0:
        logger.warning(
            f"Dropped {dropped} cases without matching image+mask files in {images_dir} / {masks_dir}."
        )
